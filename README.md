...
- inferred Typing
  ...
  ```js
  let userName = 'smith'; // data type inferred as string
  ```
...
2. **object**: Represents any non-primitive value.
   ...
   user1 = { userName: 'smith', userId: 101 };
   users.push(user1);

   let user2: { userName: string; userId: number };
   user2 = { userName: 'leo', userId: 102 };
...
3. **array**: Represents an array of values of a specific type. 2 ways to declare: `number[]` or `Array<number>`
   ...
   // let users = ["smith", "leo", "devit"];

   // let users: string[];
   // users = ["smith", "leo", "devit"];
...
8. **class Type OOP** - You can create class type as well. class can have constructor, properties, methods. create object  let objectName = new className()
    ...
    let user2 = new User('Rabeya deo', 31);
    user2.display();
    ```
...
10. **Interface vs type**
    ...
    const s1 = new Student('john doe', '1302020017');
...
- Function signature
  ...
  userInfo1 = () => {
    console.log('john doe is 32 years old');
  };
  ...
  userInfo1();
  userInfo2('john doe');
  console.log(userInfo3('john doe'));
  ```
